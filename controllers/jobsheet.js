//mengambil database/modelnya dan tabel quiz
const db = require('../models')
const Quiz = db.quizzes

exports.submitOne = async (req, res) => {    
    const jobsheet = {
        //request bodynya quizId dan answer
        quizId: req.body.quizId,
        answer: req.body.answer,
    }

    try {
        //membuat variabel quiz yg memanggil perintah  findOne
        ...
        if (req.body.answer == quiz.key) {
            res.status(200).json({
                'message': `jawabanmu adalaah ${quiz.answer} dan benar`
            })
        }
        else {
            res.status(200).json({
